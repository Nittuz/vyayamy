...
export async function pullOnce(): Promise<void> {
  ...
  async function pullTable(table: SyncedTable): Promise<void> {
    ...
    let cursor = meta?.last_pulled_at ?? EPOCH;
    ...
    while (true) {
      ...
      await db.withTransactionAsync(async () => {
        for (const row of data) {
          await upsertLocal(table, row as Record<string, unknown>);
        }
      });

      const last = data[data.length - 1] as Record<string, unknown> | undefined;
      const next = (last?.updated_at as string | undefined) ?? cursor;
      await db.runAsync(
