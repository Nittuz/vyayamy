...
- Staging the **first set of an exercise** (next-exercise advance in `useWorkoutCursor.ts:87`, picker add, workout start) queries local history: the most recent _completed_ workout containing the same `exerciseId`. Seed = that session's same-index set, falling back to its top set. Weight converts via `convertWeight` to the current profile unit and rounds to the current step. New pure helper (e.g. `planFirstSet`) beside `planStagedSet` in `activeSet.ts`; the history lookup is a query-layer function with its own unit test.
