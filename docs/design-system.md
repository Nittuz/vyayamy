...
export function LastSession({ onRepeat }: Props) {
  ...
  return (
    <Plate onPress={onRepeat} faceStyle={styles.face} accessibilityRole="button">
      <Text variant="label" color={theme.color.inkTertiary}>Last — push day</Text>
      <Text variant="numeral" color={theme.color.ink}>80 kg × 8</Text>
    </Plate>
  );
}
