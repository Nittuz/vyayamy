...
**Update 2026-06-10:** the deep review (finding #8) showed `now()` is transaction-start time, so `updated_at` is not strictly monotonic in commit order. Two overlapping write transactions can commit out of timestamp order, opening a read-skew window at the head of the cursor. Since commit 3fe61d6 the pull engine rewinds its read cursor by a 5s overlap (`CURSOR_OVERLAP_MS` in src/sync/pull.ts) on every pull; the merge is an idempotent upsert, so re-seeing rows is harmless. The trigger plus this rewind together provide the correctness this ADR claims.
